...
collapse (mean) mean_pm25, by(adm2_name year)

estpost tabstat mean_pm25 , c(stat) stat(n mean sd min max)

esttab using "/Users/shashanksingh/Desktop/air_pollution/results/tables/Table1a_SummaryStats_yearly.tex", replace cells("count(fmt(0)) mean(fmt(%15.2fc %15.2fc %15.0fc %15.0fc)) sd min max") label booktabs f noobs nonumbers nomtitles collabels("N" "Mean" "SD" "Min" "Max", end(" \midrule \\ \multicolumn{5}{l}{\emph{Monitor-Year Level}} \\"))
...
esttab using "/Users/shashanksingh/Desktop/air_pollution/results/tables/Table1b_SummaryStats_yearly.tex", replace cells("count(fmt(0)) mean(fmt(%15.2fc %15.2fc %15.2fc %15.2fc %15.2fc 0 %15.2fc)) sd min max") label booktabs f noobs nomtitles nonumbers collabels(none) mgroups("\emph{Case Level Data - Pollution}", pattern(1 0 0 0 0) prefix( \\ \multicolumn{@span}{l}{) suffix(}) span)
...
esttab using "/Users/shashanksingh/Desktop/air_pollution/results/tables/Table1b_SummaryStats_yearly.tex", append cells("count(fmt(0)) mean(fmt(%15.2fc %15.2fc %15.2fc %15.2fc 2)) sd min max") label booktabs f noobs nomtitles nonumbers collabels(none) mgroups("\emph{Judge Level Data (Pollution Sample)}", pattern(1 0 0 0 0) prefix( \\ \multicolumn{@span}{l}{) suffix(}) span) 
