...
* Load Data
import delimited "$Data\processed_data\monthly_juris_air_olexiy_jidentity_standardised.csv",varnames(1) case(lower) bindquote(strict) maxquotedrows(200) clear 
...
collapse (sum) EnvCase NotGreenCaseMedian GreenCaseMedian NotGreenCaseMean GreenCaseMean (mean) bc du oc14 oc16 oc18 pm14 pm16 pm18 so2 so4 ss NumJudges JudgeWoman JudgeGrad JudgePostGrad GovtPetitioner GovtRespondent Appeal Constitutional lsa* Onelsa* d2v_vec* jidentity_*, by(district year month)
...
forvalues p = 1/5{
	...
	by district: gen jidentity_*_lag_`p' = jidentity_*[_n-`p']
...
save "$Data\temp_data\tempdata_monthly_air_olexiy_mhml.csv", replace
